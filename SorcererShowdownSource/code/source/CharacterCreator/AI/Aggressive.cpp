...
bool Aggressive::TryDomainActions(CurseUser* user, Battlefield& bf, Character*) {
    ...
    if (!domain_users.empty()) {
        ...
        if (Utilities::GetRandomNumber(1, 100) <= 25 && user->GetDomain() && !user->DomainActive() && !user->IsStrained() && user->GetDomain()->GetDomainUses() < 5 && (!user->GetTechnique() || !user->GetTechnique()->BurntOut())) {
            user->ActivateDomain(); 
            return true; 
        }
    }
