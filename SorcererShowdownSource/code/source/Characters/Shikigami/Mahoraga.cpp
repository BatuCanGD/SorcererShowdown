...
void Mahoraga::PrintStatus(CurseUser* s) const {
    if (PrevState != InfStage) {
        ...
        case InfinityAdaptation::FirstSpin: std::println("{}'s {} has started to adapt space itself!!!!!", s->GetNameWithID(), name); break;
        case InfinityAdaptation::SecondSpin: std::println("{}'s {} is on its second spin to adapt space itself!!!!!", s->GetNameWithID(), name); break;
        case InfinityAdaptation::ThirdSpin: std::println("{}'s {} is on its final spin to adapt space itself!!!!!",s->GetNameWithID(), name); break;
        case InfinityAdaptation::FourthSpin: std::println("{}'s {} has adapted to space itself!!!!!", s->GetNameWithID(), name); break;
        default:
            break;
        }
    }
...
void Mahoraga::OnShikigamiTurn(CurseUser* user, Battlefield&) {
    ...
    if (user->GetCharacterCE() < keep_active_cost) {
        std::println("{} cannot maintain its active state due to {}'s insufficient {}Cursed Energy!{} It withdraws back into the shadows", name, user->GetNameWithID(), Color::Cyan, Color::Clear);
        Withdraw();
        return;
    }
    IncrementActiveTime();
