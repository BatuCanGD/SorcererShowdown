    ...
    for (const auto& s : bf.battlefield) {
        ...
        if (s.get() == user) {
            ...
            if (s->GetCharacterHealth() <= 0.0) {
                std::println("{} got hit and defeated by their own {}unlimited hollow purple!{}",s->GetNameWithID() ,Color::Purple,Color::Clear);
            }
            else {
                std::println("{} took the hit and received{} {:.1f} damage!{}",s->GetNameWithID(), Color::Red, (damage * 0.15) / usrn, Color::Clear);
            }
            continue;
        }
        s->DamageBypass(damage);
        double trrn = target->GetDamageReinforcement();
        std::println("{} got hit by Unlimited Hollow Purple for {}{:.1f} damage!{}", s->GetNameWithID(), Color::Red, damage / trrn, Color::Clear);
    }
    use_amount++;
